    ...
    <div class="col-xs-12">
        <div>
            <table id="tax-rate-table"
                ...
                <tbody>
                <c:forEach items="${taxRateInfoList }" var="n">
                    <tr>
                        <td width="100px">${n.id}</td>
                        <td width="200px">${n.taxTypeName}</td>
                        <td width="100px">${n.rate}</td>
                        <td width="100px">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                ...
                <h4 id='modal-title' class="blue bigger">添加/修改税率信息</h4>
            </div>
                        ...
                        <form class="form-horizontal" role="form">
                            ...
                            <div class="form-group">
                                <label class="col-xs-2 control-label no-padding-right"
                                       for="tax-type-name">征税类型<font color="red">*</font>
                                </label>

                                <div class="col-xs-4">
                                    <input type="text" id="tax-type-name" placeholder="征税类型"
                                           name="tax-type-name" class="input-large">
                                </div>
                                <label class="col-xs-2 control-label no-padding-right"
                                       for="tax-rate">税率<font color="red">*</font></label>

                                <div class="col-xs-4">
                                    <input type="text" id="tax-rate" placeholder="税率"
                                           name="tax-rate" class="input-large">
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    $('.page-content-area').ace_ajax('loadScripts', scripts, function () {
        ...
        jQuery(function ($) {
            var dt = $('#tax-rate-table').dataTable({
                bAutoWidth: true,
                "aaSorting": [],
                "oLanguage": {
            ...
            $(document).on('click', 'td div .green i', function () {
                ...
                $('#modal-form-add #tax-type-name').val($(ttr).find('td')[1].textContent);
                $('#modal-form-add #tax-rate').val($(ttr).find('td')[2].textContent);
            });
            ...
            $("#save").bind("click", function () {
                ...
                if (x == true) {
                    ...
                    arr = str.split("/");
                    urlString = "${contextPath}/sys/taxrateinfo/save";
                    $.ajax({
                        dataType: "json",
                        url: urlString,
                        type: "post",
                        data: {
                            ...
                            taxTypeName: $("form #tax-type-name").val(),
                            rate: $("form #tax-rate").val(),
                            cmd: $("#flag").val()
                        },
            ...
            $(document).on('click', 'td div .red i', function () {
                var ttr = $(this).closest('tr');
                urlString = "${contextPath}/sys/taxrateinfo/delete/" +
