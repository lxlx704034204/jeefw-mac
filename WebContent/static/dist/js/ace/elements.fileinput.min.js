(function ($, undefined) {
    ...
    var hasFileList = "FileList" in window;
    ...
    var Ace_File_Input = function (element, settings) {
        ...
        this.disabled = false;
        this.can_reset = true;
        this.$element.off("change.ace_inner_call").on("change.ace_inner_call", function (e, ace_inner_call) {
            if (self.disabled) {
                return
            }
    ...
    Ace_File_Input.error = {"FILE_LOAD_FAILED": 1, "IMAGE_LOAD_FAILED": 2, "THUMBNAIL_FAILED": 3};
    Ace_File_Input.prototype.apply_settings = function () {
        ...
        if (remove_btn) {
            var btn = $('<a class="remove" href="#"><i class="' + ace.vars["icon"] + this.settings.icon_remove + '"></i></a>').appendTo(this.$element.parent());
    ...
    Ace_File_Input.prototype.show_file_list = function ($files, inner_call) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (typeof files[i] === "string") {
                ...
                if (hasFile && files[i] instanceof File) {
                    filename = $.trim(files[i].name)
                } else {
                    if (files[i] instanceof Object && files[i].hasOwnProperty("name")) {
            ...
            if (format == false) {
                if ((/\.(jpe?g|png|gif|svg|bmp|tiff?)$/i).test(filename)) {
                    ...
                    if ((/\.(mpe?g|flv|mov|avi|swf|mp4|mkv|webm|wmv|3gp)$/i).test(filename)) {
                        format = "video"
                    } else {
                        if ((/\.(mp3|ogg|wav|wma|amr|aac)$/i).test(filename)) {
                            format = "audio"
                        } else {
                            format = "file"
                        }
            ...
            if (!this.well_style) {
                ...
                var can_preview = hasFileReader && this.settings.thumbnail && ((type.length > 0 && type.match("image")) || (type.length == 0 && format == "image"));
                if (can_preview) {
                    var self = this;
    ...
    Ace_File_Input.prototype.reset_input_ui = function () {
        ...
        if (!this.settings.no_icon) {
            this.$label.find(ace.vars[".icon"]).remove()
        }
    ...
    var enable_drop_functionality = function () {
        ...
        dropbox.off("dragenter").on("dragenter", function (e) {
            ...
            if (!self.multi && file_list.length > 1) {
                var tmpfiles = [];
                tmpfiles.push(file_list[0]);
                file_list = tmpfiles
            }
            file_list = processFiles.call(self, file_list, true);
            if (file_list === false) {
                return false
            }
            self.$element.data("ace_input_method", "drop");
            self.$element.data("ace_input_files", file_list);
            self.show_file_list(file_list, true);
            self.$element.triggerHandler("change", [true]);
            return true
        })
    };
    var handle_on_change = function () {
        var file_list = this.element.files || [this.element.value];
        file_list = processFiles.call(this, file_list, false);
        if (file_list === false) {
            return false
        }
    ...
    var preview_image = function (file) {
        var self = this;
        var $span = self.$label.find(".ace-file-name:last");
        ...
        var imgLoaded = function (img) {
            ...
            if (self.settings.thumbnail == "large") {
                ...
                if (self.settings.thumbnail == "fit") {
                    size = $span.width()
                }
            }
    ...
    var processFiles = function (file_list, dropped) {
        ...
        if (this.settings.before_change) {
            ret = this.settings.before_change.call(this.element, file_list, dropped)
        }
        if (ret === -1) {
            this.reset_input();
            return false
    ...
    var checkFileList = function (files, dropped) {
        ...
        if (!(allowExt || denyExt || allowMime || denyMime || maxSize)) {
            return true
        }
        var safe_files = [];
        var error_list = {};
        for (var f = 0; f < files.length; f++) {
            ...
            if (allowExt && !allowExt.test(filename)) {
                if (!("ext" in error_list)) {
                    error_list["ext"] = []
                }
                error_list["ext"].push(filename);
                continue
            } else {
                if (denyExt && denyExt.test(filename)) {
                    if (!("ext" in error_list)) {
                        error_list["ext"] = []
                    }
                    error_list["ext"].push(filename);
            ...
            if (!hasFile) {
                ...
                if ((type = $.trim(file.type)).length > 0) {
                    if (allowMime && !allowMime.test(type)) {
                        if (!("mime" in error_list)) {
                            error_list["mime"] = []
                        }
                        error_list["mime"].push(filename);
                        continue
                    } else {
                        if (denyMime && denyMime.test(type)) {
                            if (!("mime" in error_list)) {
                                error_list["mime"] = []
                            }
                            error_list["mime"].push(filename);
        ...
        if (safe_files.length == files.length) {
            return files
        }
        var error_count = {"ext": 0, "mime": 0, "size": 0};
        if ("ext" in error_list) {
            error_count["ext"] = error_list["ext"].length
        }
        if ("mime" in error_list) {
            error_count["mime"] = error_list["mime"].length
        }
        if ("size" in error_list) {
            error_count["size"] = error_list["size"].length
        }
        var event;
        this.$element.trigger(event = new $.Event("file.error.ace"), {
