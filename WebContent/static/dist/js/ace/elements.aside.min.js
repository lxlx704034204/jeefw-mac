(function ($, undefined) {
    ...
    function Aside(modal, settings) {
        ...
        this.initiate = function () {
            ...
            if (placement) {
                placement = $.trim(placement.toLowerCase())
            } else {
                placement = "right"
            }
            backdrop = settings.backdrop || $modal.attr("data-backdrop");
            if (!(/right|top|left|bottom/.test(placement))) {
                placement = "right"
            }
            ...
            if (/right|left/.test(placement)) {
                vertical = true;
                $modal.addClass("aside-vc")
            } else {
                $modal.addClass("aside-hz")
            }
            if (options.fixed) {
                $modal.addClass("aside-fixed")
            }
            if (options.dark) {
                $modal.addClass("aside-dark")
            }
            if (options.offset) {
                $modal.addClass("navbar-offset")
            }
            if (!options.transition) {
                $modal.addClass("transition-off")
            }
            ...
            if (backdrop === false || backdrop === "false") {
                ...
                if (backdrop === "invisible") {
                    invisible_backdrop = true
                }
            }
        };
        this.show = function () {
            $modal.css("position", "fixed").removeClass("aside-hidden")
        };
        this.hide = function () {
            toggleButton();
        ...
        this.shown = function () {
            ...
            $window.off("resize.modal.aside").on("resize.modal.aside", function () {
                if (!ace.vars["touch"]) {
                    content.ace_scroll("disable");
                    var size = !vertical ? dialog.height() : content.height();
                    content.ace_scroll("update", {"size": size}).ace_scroll("enable").ace_scroll("reset")
                } else {
                    content.css("max-height", (!vertical ? dialog.height() : content.height()) + "px")
                }
    ...
    $(document).on("show.bs.modal", ".modal.aside", function (e) {
        $(".aside.in").modal("hide");
        $(this).ace_aside("show")
    ...
    $.fn.aceAside = $.fn.ace_aside = function (option, value) {
        ...
        var $set = this.each(function () {
            ...
            if (!data) {
                $this.data("ace_aside", (data = new Aside(this, options)))
            }
