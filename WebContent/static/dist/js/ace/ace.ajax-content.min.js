(function ($, undefined) {
    ...
    function AceAjax(contentArea, options) {
        ...
        this.getUrl = function (url, hash, manual_trigger) {
            ...
            $.ajax({"url": url}).error(function () {
                ...
                if (typeof update_breadcrumbs === "function") {
                    ...
                    if (update_breadcrumbs === true && link_element != null && link_element.length > 0) {
                        link_text = updateBreadcrumbs(link_element)
                    }
                }
                result = String(result).replace(/<(title|link)([\s\>])/gi, '<div class="hidden ajax-append-$1"$2').replace(/<\/(title|link)\>/gi, "</div>");
        ...
        $(window).off("hashchange.ace_ajax").on("hashchange.ace_ajax", function (e, manual_trigger) {
