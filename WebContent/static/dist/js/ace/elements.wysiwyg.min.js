(function ($, undefined) {
    $.fn.ace_wysiwyg = function ($options, undefined) {
        var options = $.extend({speech_button: true, wysiwyg: {}}, $options);
        var color_values = ["#ac725e", "#d06b64", "#f83a22", "#fa573c", "#ff7537", "#ffad46", "#42d692", "#16a765", "#7bd148", "#b3dc6c", "#fbe983", "#fad165", "#92e1c0", "#9fe1e7", "#9fc6e7", "#4986e7", "#9a9cff", "#b99aff", "#c2c2c2", "#cabdbf", "#cca6ac", "#f691b2", "#cd74e6", "#a47ae2", "#444444"];
        var button_defaults = {
            "font": {
            ...
            "foreColor": {values: color_values, title: "Change Color"},
            "backColor": {values: color_values, title: "Change Background Color"},
        ...
        var toolbar_buttons = options.toolbar || ["font", null, "fontSize", null, "bold", "italic", "strikethrough", "underline", null, "insertunorderedlist", "insertorderedlist", "outdent", "indent", null, "justifyleft", "justifycenter", "justifyright", "justifyfull", null, "createLink", "unlink", null, "insertImage", null, "foreColor", null, "undo", "redo", null, "viewSource"];
        this.each(function () {
            ...
            for (var tb in toolbar_buttons) {
                if (toolbar_buttons.hasOwnProperty(tb)) {
                    ...
                    if (typeof button == "string" && button in button_defaults) {
                        ...
                        if (typeof button == "object" && button.name in button_defaults) {
                    ...
                    switch (button.name) {
                        ...
                        case"createLink":
                            ...
                            toolbar += ' <div class="dropdown-menu dropdown-caret dropdown-menu-right">							 <div class="input-group">								<input class="form-control" placeholder="' + button.placeholder + '" type="text" data-edit="' + button.name + '" />								<span class="input-group-btn">									<button class="btn btn-sm ' + button.button_class + '" type="button">' + button.button_text + "</button>								</span>							 </div>						</div> </div>";
                            break;
                        case"insertImage":
                            toolbar += ' <div class="btn-group"> <a class="btn btn-sm ' + className + ' dropdown-toggle" data-toggle="dropdown" title="' + button.title + '"><i class="' + ace.vars["icon"] + button.icon + '"></i></a> ';
                            toolbar += ' <div class="dropdown-menu dropdown-caret dropdown-menu-right">							 <div class="input-group">								<input class="form-control" placeholder="' + button.placeholder + '" type="text" data-edit="' + button.name + '" />								<span class="input-group-btn">									<button class="btn btn-sm ' + button.button_insert_class + '" type="button">' + button.button_insert + "</button>								</span>							 </div>";
                            if (button.choose_file && "FileReader" in window) {
                                toolbar += '<div class="space-2"></div>							 <label class="center block no-margin-bottom">								<button class="btn btn-sm ' + button.button_class + ' wysiwyg-choose-file" type="button">' + button.button_text + '</button>								<input type="file" data-edit="' + button.name + '" />							  </label>'
                            }
                            toolbar += " </div> </div>";
                            break;
                        case"foreColor":
