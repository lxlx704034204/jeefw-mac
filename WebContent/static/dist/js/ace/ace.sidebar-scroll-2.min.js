(function ($, undefined) {
    ...
    var is_element_pos = "getComputedStyle" in window ? function (el, pos) {
        el.offsetHeight;
    ...
    function Sidebar_Scroll(sidebar, settings) {
        var self = this;
        var $window = $(window);
        var $sidebar = $(sidebar), $nav = $sidebar.find(".nav-list"), $toggle = $sidebar.find(".sidebar-toggle").eq(0), $shortcuts = $sidebar.find(".sidebar-shortcuts").eq(0), nav = $nav.get(0);
        if (!nav) {
            return
        }
        ...
        var scroll_div = null, scroll_content = null, scroll_content_div = null, bar = null, ace_scroll = null;
        this.is_scrolling = false;
        var _initiated = false;
        this.sidebar_fixed = is_element_pos(sidebar, "fixed");
        var $avail_height, $content_height;
        var scroll_to_active = settings.scroll_to_active || ace.helper.boolAttr(sidebar, "data-scroll-to-active") || false, include_shortcuts = settings.include_shortcuts || ace.helper.boolAttr(sidebar, "data-scroll-include-shortcuts") || false, include_toggle = settings.include_toggle || ace.helper.boolAttr(sidebar, "data-scroll-include-toggle") || false, scroll_style = settings.scroll_style || $sidebar.attr("data-scroll-style") || "";
        ...
        this.reset = function (recalc) {
            ...
            var vars = ace_sidebar["vars"]();
            var enable_scroll = !vars["minimized"] && !vars["collapsible"] && !vars["horizontal"] && ($avail_height = available_height()) < ($content_height = nav.parentNode.scrollHeight);
            ...
            if (event_name == "sidebar_collapsed") {
                ...
                if (event_name === "sidebar_fixed" || event_name === "navbar_fixed") {
                    ...
                    if (sidebar_fixed && !is_scrolling) {
                        ...
                        if (!sidebar_fixed && sidebar_scroll.get("only_if_fixed")) {
                            sidebar_scroll.disable()
                        }
                    }
                }
    ...
    if (!$.fn.ace_sidebar_scroll) {
        $.fn.ace_sidebar_scroll = function (option, value) {
            ...
            var $set = this.each(function () {
                ...
                if (!data) {
                    $this.data("ace_sidebar_scroll", (data = new Sidebar_Scroll(this, options)))
                }
