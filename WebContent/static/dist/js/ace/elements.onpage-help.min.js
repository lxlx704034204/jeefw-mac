window.Onpage_Help = function (options) {
    ...
    var ie_fix = document.all && !window.atob;
    var section_start = {};
    var section_end = {};
    var section_rect = {};
    var section_count = 0;
    ...
    var captureFocus = function () {
        ...
        var scroll = -1;
        $(document).off("focusin.ace.help").on("focusin.ace.help", function (e) {
    ...
    this.enable = function () {
        ...
        captureFocus();
        if (typeof _.settings.after_enable === "function") {
            _.settings.after_enable.call(self)
        }
    };
    this.disable = function () {
        ...
        document.body.style.overflowX = ovfx;
        releaseFocus();
        if (typeof _.settings.after_disable === "function") {
            _.settings.after_disable.call(self)
        }
    };
    ...
    this.init = function () {
        ...
        help_container = $('<div class="onpage-help-container" id="onpage-help-container" tabindex="-1" />').appendTo("body");
    ...
    function save_sections(reset) {
        ...
        if (reset === true) {
            help_container.find(".onpage-help-section").remove()
        }
        section_start = {};
        section_end = {};
        section_count = 0;
        ...
        $(comments).each(function () {
            ...
            if ((match = $.trim(this.data).match(/#section\s*:\s*([\w\d\-\.\/]+)/i))) {
                ...
                if (!(section_name in section_start)) {
                    section_start[section_name] = this
                }
            }
            if ((match = $.trim(this.data).match(/\/section\s*:\s*([\w\d\-\.\/]+)/i))) {
                var section_name = match[1];
    ...
    function save_section_offset(name) {
        if (!(name in section_start) || !(name in section_end)) {
            return
        }
    ...
    function highlight_section(name) {
        if (!(name in section_rect) || !help_container) {
            return
        }
        ...
        if (div.length == 0) {
            ...
            if (ie_fix) {
                div.append('<span class="ie-hover-fix" />')
            }
            if (_.settings.icon_1) {
                div.append('<i class="help-icon-1 ' + _.settings.icon_1 + '"></i>')
            }
            if (_.settings.icon_2) {
                div.append('<i class="help-icon-2 ' + _.settings.icon_2 + '"></i>')
            }
        ...
        div.css({left: rect.left, top: rect.top, width: rect.width, height: rect.height});
        ...
        if (rect.height < 55 || rect.width < 55) {
            ...
            if (rect.height < 75 || rect.width < 75) {
    ...
    function disableBodyScroll() {
        ...
        mbody.parent().off(mousewheel_event).on(mousewheel_event, function (event) {
            if (disableScroll) {
                event.preventDefault()
            } else {
                event.deltaY = event.deltaY || 0;
                var delta = (event.deltaY > 0 || event.originalEvent.detail < 0 || event.originalEvent.wheelDelta > 0) ? 1 : -1;
                if (delta == -1 && body.scrollTop + body.clientHeight >= body.scrollHeight) {
                    event.preventDefault()
                } else {
                    if (delta == 1 && body.scrollTop <= 0) {
                        event.preventDefault()
                    }
                }
            }
    ...
    function launch_help_modal(section_name, save_to_list) {
            ...
            help_modal.find(".onpage-help-modal-buttons").on("click", "button[data-navdir]", function () {
                ...
                if (dir == "back") {
                    ...
                    if (dir == "forward") {
                        ...
                        if (dir == "up") {
                            var $this = $(this), url;
                            if ($this.hasClass("disabled") || !(url = $this.attr("data-url"))) {
                                return
                            }
        ...
        if (!help_modal.hasClass("in")) {
            if (document.body.lastChild != help_modal[0]) {
                $(document.body).append(help_modal)
            }
        ...
        if (typeof _.settings.section_url === "function") {
            url = _.settings.section_url.call(self, section_name)
        }
        $.ajax({url: url, dataType: "text"}).done(function (result) {
            var title = "", excerpt = "";
            if (typeof _.settings.section_title === "function") {
                title = _.settings.section_title.call(self, result, section_name, url)
            } else {
            ...
            if (typeof _.settings.section_content === "function") {
                excerpt = _.settings.section_content.call(self, result, section_name, url)
            } else {
            ...
            if (typeof _.settings.code_highlight === "function") {
                ...
                excerpt = excerpt.replace(/\<pre((?:(?:.*?)(?:data\-language=["'](?:[\w\d]+)["'])(?:.*?))|(?:(?:.*?)(?:class=["'](?:.*?)language\-(?:[\w\d]+)(?:.*?)["'])(?:.*?)))\>([\s\S]+?)\<\/pre\>/ig, function (a, b, c) {
            ...
            if (typeof _.settings.code_highlight === "function") {
                ...
                if (_.settings.code_highlight === "rainbow") {
                    try {
                        Rainbow.color(content[0])
                    } catch (e) {
                    }
                } else {
                    if (_.settings.code_highlight === "prism") {
                        try {
            ...
            if (_.settings.add_panels) {
                content.find(_.settings.panel_content_selector).each(function () {
                    var header = $(this).prevAll(_.settings.panel_content_title);
                    if (header.length == 0) {
                        return false
                    }
                    header = header.attr("class", "panel-title").wrapInner('<a class="help-panel-toggle" href="#" data-parent="#" data-toggle="collapse" />').wrap('<div class="panel-heading" />').closest(".panel-heading");
                ...
                content.find(".panel").each(function () {
                    ...
                    group.find(".help-panel-toggle").append('<i class="pull-right ace-icon fa fa-plus" data-icon-show="ace-icon fa fa-plus" data-icon-hide="ace-icon fa fa-minus"></i>').attr("data-parent", "#" + group_id).each(function () {
                        panel_id++;
    ...
    function display_codeview(url, language, save_to_list) {
        var $url = url;
        if (typeof _.settings.file_url === "function") {
            url = _.settings.file_url.call(self, url, language)
        }
        $.ajax({url: url, dataType: "text"}).done(function (result) {
