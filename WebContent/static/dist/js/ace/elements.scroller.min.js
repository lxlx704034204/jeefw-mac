(function ($, undefined) {
    var Ace_Scroll = function (element, _settings) {
        ...
        var trackFlip = false;
        var trackSize = 0;
        var css_pos, css_size, max_css_size, client_size, scroll_direction, scroll_size;
        var ratio = 1;
        ...
        var detached = settings.detached || false;
        var updatePos = settings.updatePos || false;
        ...
        this.create = function (_settings) {
            ...
            if (_settings) {
                settings = $.extend({}, $.fn.ace_scroll.defaults, _settings)
            }
            this.size = parseInt(this.$element.attr("data-size")) || settings.size || 200;
            vertical = !settings["horizontal"];
            css_pos = vertical ? "top" : "left";
            css_size = vertical ? "height" : "width";
            ...
            $content_wrap.on("scroll", function () {
                ...
                move_bar = false;
                if (trigger_scroll) {
                    this.$element.trigger("scroll", [content_wrap])
                }
            });
            if (settings.mouseWheel) {
                ...
                this.$element.on(!!$.event.special.mousewheel ? "mousewheel.ace_scroll" : "mousewheel.ace_scroll DOMMouseScroll.ace_scroll", function (event) {
                    ...
                    if (!self.lock) {
                        if (delta == -1) {
                            scrollEnd = (content_wrap[scroll_size] <= scrollAmount + clientSize)
                        } else {
            ...
            var touchDrag = ace.vars["touch"] && "ace_drag" in $.event.special && settings.touchDrag;
            if (touchDrag) {
                ...
                this.$element.on(event_name + ".ace_scroll", function (event) {
                    ...
                    dir = event.direction;
                    if ((vertical && (dir == "up" || dir == "down")) || (!vertical && (dir == "left" || dir == "right"))) {
                        var distance = vertical ? event.dy : event.dx;
                        if (distance != 0) {
                            if (Math.abs(distance) > 20 && touchDrag) {
                                distance = distance * 2
                            }
                            self.move_bar(true);
            ...
            if (!vertical) {
                $content_wrap.children(0).css(css_size, this.size)
            }
        ...
        this.reset = function (innert_call) {
            ...
            is_dirty = false;
            if (detached) {
                var border_size = parseInt(Math.round((parseInt($content_wrap.css("border-top-width")) + parseInt($content_wrap.css("border-bottom-width"))) / 2.5));
                size -= border_size
            }
            ...
            if (!vertical) {
                $content_wrap.children(0).css(css_size, size)
            }
            $content_wrap.css(max_css_size, this.size);
        ...
        this.destroy = function () {
            ...
            if (inline_style !== false) {
                this.element.style.position = inline_style
            }
            if (idleTimer != null) {
        ...
        function mouse_down_track(e) {
            ...
            var track_pos = track_offset[css_pos];
        ...
        function mouse_down_bar(e) {
            ...
            if (dragEvent) {
                self.$element.trigger("drag.start")
            }
        }
        ...
        function mouse_move_bar(e) {
            ...
            if (mouse_pos2 - mouse_pos1 + bar_pos > bar_max_pos) {
                ...
                if (mouse_pos2 - mouse_pos1 + bar_pos < 0) {
        ...
        function mouse_up_bar(e) {
            ...
            if (active && hideOnIdle && !insideTrack) {
                hideScrollbars()
            }
        }
        ...
        function getTrackSize() {
            ...
            if (vertical) {
                trackSize = parseInt($track.outerWidth()) || 0
            } else {
                trackSize = parseInt($track.outerHeight()) || 0
            }
        ...
        function setTrackPos() {
            ...
            if (updatePos === true) {
                $track.css({top: parseInt(top), left: parseInt(left)})
            } else {
                if (updatePos === "left") {
                    $track.css("left", parseInt(left))
                } else {
                    if (updatePos === "top") {
                        $track.css("top", parseInt(top))
                    }
                }
            }
        }
    ...
    $.fn.ace_scroll = function (option, value) {
        ...
        var $set = this.each(function () {
            ...
            if (!data) {
                $this.data("ace_scroll", (data = new Ace_Scroll(this, options)))
            }
