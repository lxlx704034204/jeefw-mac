...
!function (a) {
    ...
    function d() {
        a.fn.ace_ajax && a("[data-ajax-content=true]").ace_ajax({
            close_active: !0, content_url: function (a) {
                if (!a.match(/^page\//))return !1;
                var b = document.location.pathname;
                return b.match(/(\/ajax\/)(ajax\.html)?/) ? b.replace(/(\/ajax\/)(ajax\.html)?/, "/ajax/" + a.replace(/^page\//, "") + ".html") : b + "?" + a.replace(/\//, "=")
...
}), ace.helper.redraw = function (a, b) {
    ...
    function c(c, e) {
        ...
        function g(b) {
            var c = h.find(".ajax-append-title");
        ...
        var h = a(c), i = this, j = e.content_url || !1, k = e.default_url || !1, l = e.loading_icon || "fa-spinner fa-2x orange", m = e.loading_text || "", n = e.update_breadcrumbs || e.update_breadcrumbs === b, o = e.update_title || e.update_breadcrumbs === b, p = e.update_active || e.update_breadcrumbs === b, q = e.close_active || !1, r = e.max_load_wait || !1, s = !1;
        this.loadUrl = function (a) {
            var b = !1;
            a = a.replace(/^(\#\!)?\#/, ""), "function" == typeof j && (b = j(a)), "string" == typeof b && this.getUrl(b, a, !1)
        }, this.getUrl = function (b, c, d) {
            if (!s) {
                var e;
                h.trigger(e = a.Event("ajaxloadstart"), {
                    url: b,
                    hash: c
                }), e.isDefaultPrevented() || (i.startLoading(), a.ajax({url: b}).error(function () {
                    h.trigger("ajaxloaderror", {url: b, hash: c}), i.stopLoading(!0)
                }).done(function (e) {
                    h.trigger("ajaxloaddone", {url: b, hash: c});
                    var j = null, k = "";
                    if ("function" == typeof p)j = p.call(null, c, b); else if (p === !0 && (j = a('a[data-url="' + c + '"]'), j.length > 0)) {
                        var l = j.closest(".nav");
                        if (l.length > 0) {
                            l.find(".active").each(function () {
                                var b = "active";
                                (a(this).hasClass("hover") || q) && (b += " open"), a(this).removeClass(b), q && a(this).find(" > .submenu").css("display", "")
                            });
                            {
                                j.closest("li").addClass("active").parents(".nav li").addClass("active open")
                            }
                            l.closest(".sidebar[data-sidebar-scroll=true]").each(function () {
                                var b = a(this);
                                b.ace_sidebar_scroll("reset"), d && b.ace_sidebar_scroll("scroll_to_active")
                            })
                        }
                    }
                    "function" == typeof n ? k = n.call(null, c, b, j) : n === !0 && null != j && j.length > 0 && (k = f(j)), e = String(e).replace(/<(title|link)([\s\>])/gi, '<div class="hidden ajax-append-$1"$2').replace(/<\/(title|link)\>/gi, "</div>"), h.empty().html(e), h.css("opacity", .6), setTimeout(function () {
                        ...
                        h.find(".ajax-append-link").each(function () {
                            var b = a(this);
                            if (b.attr("href")) {
                                var d = jQuery("<link />", {
                                    type: "text/css",
                    ...
                    }, 10), "function" == typeof o ? o.call(null, c, b, k) : o === !0 && g(k), d || a("html,body").animate({scrollTop: 0}, 250), h.trigger("ajaxloadcomplete", {
                        url: b,
                        hash: c
                    }), i.stopLoading()
                }))
        ...
        this.startLoading = function () {
            s || (s = !0, h.css("opacity", .25).prevAll(".ajax-loading-overlay").remove(), a('<div class="ajax-loading-overlay"><i class="ajax-loading-icon fa fa-spin ' + l + '"></i> ' + m + "</div>").insertBefore(c), r !== !1 && (t = setTimeout(function () {
                if (t = null, s) {
                    var b;
                    h.trigger(b = a.Event("ajaxloadlong")), b.isDefaultPrevented() || i.stopLoading(!0)
                }
            }, 1e3 * r)))
        }, this.stopLoading = function (a) {
            a === !0 ? (s = !1, h.css("opacity", 1).prevAll(".ajax-loading-overlay").remove(), null != t && (clearTimeout(t), t = null)) : (h.css("opacity", .75), h.one("ajaxscriptsloaded", function () {
                i.stopLoading(!0)
            }))
            ...
            a.ajaxPrefilter("script", function (a) {
                ...
                function e() {
                    "function" == typeof c && c(), a('.btn-group[data-toggle="buttons"] > .btn').button(), h.trigger("ajaxscriptsloaded")
                }

                function f(a) {
                    a += 1, a < b.length ? g(a) : e()
                }
                ...
                for (var i = 0, j = 0, k = 0; k < b.length; k++)b[k] && !function () {
                    var a = "js-" + b[k].replace(/[^\w\d\-]/g, "-").replace(/\-\-/g, "-");
                    d[a] !== !0 && i++
                }();
                i > 0 ? g() : e()
            }, 10)
            ...
            b && 0 != b.length && i.loadUrl(b)
        }).trigger("hashchange.ace_ajax", [!0]);
        var u = a.trim(window.location.hash);
        !u && k && (window.location.hash = k)
    }

    var d = {};
    a.fn.aceAjax = a.fn.ace_ajax = function (d, e, f) {
        var g, h = this.each(function () {
            var h = a(this), i = h.data("ace_ajax"), j = "object" == typeof d && d;
            i || h.data("ace_ajax", i = new c(this, j)), "string" == typeof d && "function" == typeof i[d] && (g = f != b ? i[d](e, f) : i[d](e))
        });
        return g === b ? h : g
    }
    ...
    function c(c, e) {
        ...
        var h = e.duration || ace.helper.intAttr(c, "data-submenu-duration") || 300;
        this.minimized = !1, this.collapsible = !1, this.horizontal = !1, this.mobile_view = !1, this.vars = function () {
            return {
        ...
        this.toggleMenu = function (a, b) {
            ...
            if (d && 0 != d.length) {
                ...
                if (d.hasClass("dropdown-toggle")) {
                    ...
                    if (!f)return !1;
                    var i = a(f), j = 0, k = f.parentNode.parentNode;
                    if (e && k == c || i.parent().hasClass("hover") && "absolute" == i.css("position") && !g.collapsible)return !1;
                    var l = 0 == f.scrollHeight;
                    return l && a(k).find("> .open > .submenu").each(function () {
                        this == f || a(this.parentNode).hasClass("active") || (j -= this.scrollHeight, g.hide(this, h, !1))
                    }), l ? (g.show(f, h), 0 != j && (j += f.scrollHeight)) : (g.hide(f, h), j -= f.scrollHeight), 0 != j && ("true" != g.$sidebar.attr("data-sidebar-scroll") || g.minimized || g.$sidebar.ace_sidebar_scroll("prehide", j)), !1
                }
                if ("tap" == ace.click_event && e && d.get(0).parentNode.parentNode == c) {
                    var m = d.find(".menu-text").get(0);
                    if (null != m && b.target != m && !a.contains(m, b.target))return b.preventDefault(), !1
                }
        ...
        this.show = function (b, c, d) {
            ...
            var g = function (b, c) {
                b && b.stopPropagation(), f.css({
                    ...
                    overflow: "",
                    height: ""
                }), c !== !1 && f.trigger(a.Event("shown.ace.submenu")), d !== !1 && (k = !1)
            };
            ...
            var g = function (b, c) {
                b && b.stopPropagation(), f.css({
                    ...
                    "transition-property": "",
                    "transition-duration": ""
                }).removeClass("nav-show").addClass("nav-hide"), c !== !1 && f.trigger(a.Event("hidden.ace.submenu")), d !== !1 && (k = !1)
            };
            return c > 0 && a.support.transition.end ? f.one(a.support.transition.end, g) : g(), ace.vars.android && setTimeout(function () {
                g(null, !1), ace.helper.redraw(b)
            }, c + 20), !0
        }, this.toggle = function (a, b) {
            if (b = b || h, 0 == a.scrollHeight) {
            ...
            var B, C, D = function () {
                ...
                if (!A && f.sidebar_fixed) {
                    i.wrap('<div class="nav-wrap-up pos-rel" />'), i.after("<div><div></div></div>"), i.wrap('<div class="nav-wrap" />'), v || j.css({"z-index": 1}), u || k.css({"z-index": 99}), n = i.parent().next().ace_scroll({
                        ...
                        styleClass: y,
                        touchDrag: !1
                    }).closest(".ace-scroll").addClass("nav-scroll"), s = n.data("ace_scroll"), o = n.find(".scroll-content").eq(0), p = o.find(" > div").eq(0), r = a(s.get_track()), q = r.find(".scroll-bar").eq(0), u && 0 != k.length && (i.parent().prepend(k).wrapInner("<div />"), i = i.parent()), v && 0 != j.length && (i.append(j), i.closest(".nav-wrap").addClass("nav-wrap-t")), i.css({position: "relative"}), x === !0 && n.addClass("scrollout"), m = i.get(0), m.style.top = 0, o.on("scroll.nav", function () {
                        m.style.top = -1 * this.scrollTop + "px"
                    }), i.on(a.event.special.mousewheel ? "mousewheel.ace_scroll" : "mousewheel.ace_scroll DOMMouseScroll.ace_scroll", function (a) {
                        return f.is_scrolling && s.is_active() ? n.trigger(a) : !z
                    }), i.on("mouseenter.ace_scroll", function () {
                        r.addClass("scroll-hover")
                    }).on("mouseleave.ace_scroll", function () {
                        r.removeClass("scroll-hover")
                    });
                    var c = o.get(0);
                    if (i.on("ace_drag.nav", function (b) {
                            if (!f.is_scrolling || !s.is_active())return void(b.retval.cancel = !0);
                            if (0 != a(b.target).closest(".can-scroll").length)return void(b.retval.cancel = !0);
                            if ("up" == b.direction || "down" == b.direction) {
                                s.move_bar(!0);
                                var d = b.dy;
                                d = parseInt(Math.min(B, d)), Math.abs(d) > 2 && (d = 2 * d), 0 != d && (c.scrollTop = c.scrollTop + d, m.style.top = -1 * c.scrollTop + "px")
                            }
                        }), w && i.on("touchstart.nav MSPointerDown.nav pointerdown.nav", function () {
                            i.css("transition-property", "none"), q.css("transition-property", "none")
                        }).on("touchend.nav touchcancel.nav MSPointerUp.nav MSPointerCancel.nav pointerup.nav pointercancel.nav", function () {
                            i.css("transition-property", "top"), q.css("transition-property", "top")
                        }), d && !v) {
                        var e = j.get(0);
                        e && o.on("scroll.safari", function () {
                            ace.helper.redraw(e)
                        })
                    }
                    if (A = !0, 1 == b && (f.reset(), t && f.scroll_to_active(), t = !1), "number" == typeof w && w > 0 && (i.css({
                            "transition-property": "top",
                            "transition-duration": (w / 1e3).toFixed(2) + "s"
                        }), q.css({
                            "transition-property": "top",
                            "transition-duration": (w / 1500).toFixed(2) + "s"
                        }), n.on("drag.start", function (a) {
                        ...
                        var h, l = ace.helper.winHeight();
                        a(window).on("scroll.ace_scroll", function () {
                            f.is_scrolling && s.is_active() && (h = ace.helper.winHeight(), h != l && (l = h, f.reset()))
                        })
                    }
                }
            };
            this.scroll_to_active = function () {
                if (s && s.is_active())try {
                    var a, b = l.vars(), c = h.find(".nav-list");
                    ...
                    var f = d - n.height();
                    f > 0 && (m.style.top = -f + "px", o.scrollTop(f))
                } catch (g) {
                }
            }, this.reset = function (a) {
                if (a === !0 && (this.sidebar_fixed = e(b, "fixed")), !this.sidebar_fixed)return void this.disable();
                A || F();
                var c = l.vars(), d = !c.collapsible && !c.horizontal && (B = D()) < (C = m.clientHeight);
                this.is_scrolling = !0, d && (p.css({
                    height: C,
                    width: 8
                }), n.prev().css({"max-height": B}), s.update({size: B}), s.enable(), s.reset()), d && s.is_active() ? h.addClass("sidebar-scroll") : this.is_scrolling && this.disable()
            }, this.disable = function () {
                this.is_scrolling = !1, n && (n.css({height: "", "max-height": ""}), p.css({
                    height: "",
                    width: ""
                }), n.prev().css({"max-height": ""}), s.disable()), parseInt(m.style.top) < 0 && w && a.support.transition.end ? i.one(a.support.transition.end, function () {
                    h.removeClass("sidebar-scroll"), i.off(".trans")
                }) : h.removeClass("sidebar-scroll"), m.style.top = 0
            }, this.prehide = function (a) {
                if (this.is_scrolling && !l.get("minimized"))if (E() + a < D())this.disable(); else if (0 > a) {
                    var b = o.scrollTop() + a;
                    if (0 > b)return;
                    m.style.top = -1 * b + "px"
                }
                ...
                null != s && s.update({styleClass: a})
            }, h.on("hidden.ace.submenu.sidebar_scroll shown.ace.submenu.sidebar_scroll", ".submenu", function (a) {
                a.stopPropagation(), l.get("minimized") || (f._reset(), "shown" == a.type && f.set_hover())
            }), F(!0)
        }
    }
    ...
    function c(b, c) {
        ...
        function k(c) {
            ...
            var g = null;
            o.minimized && (g = d.find(".menu-text").get(0)) && (g.style.marginTop = "");
            var h = ace.helper.scrollTop(), i = 0, k = h;
            v && (i = b.offsetTop, k += i + 1);
            var l = d.offset();
            l.top = parseInt(l.top);
            var m, p = 0;
            c.style.maxHeight = "";
            var q = c.scrollHeight, m = d.height();
            g && (p = m, l.top += p);
            var u = parseInt(l.top + q), x = 0, y = t.height(), z = parseInt(l.top - k - p), A = y, B = o.horizontal, C = !1;
            B && this.parentNode == n && (x = 0, l.top += d.height(), C = !0), !C && (x = u - (y + h)) >= 0 && (x = z > x ? x : z, 0 == x && (x = 20), z - x > 10 && (x += parseInt(Math.min(25, z - x))), l.top + (m - p) > u - x && (x -= l.top + (m - p) - (u - x)), x > 0 && (c.style.top = -x + "px", g && (g.style.marginTop = -x + "px"))), 0 > x && (x = 0);
            var D = x > 0 && x > m - 20;
            ...
            var H = d.hasClass("hover") && !o.mobile_view;
            if (!(H && f.find("> li > .submenu").length > 0)) {
                var I = A - (l.top - h) + x, J = x - I;
                if (J > 0 && m > J && (I += parseInt(Math.max(m, m - J))), I -= 5, !(90 > I)) {
                    var K = !1;
                    ...
                    if (w = I, !e && K) {
                        I > 14 && q - I > 4 ? (K.enable(), K.reset()) : K.disable();
                        var L = K.get_track();
                        if (L) {
                            L.style.top = -(x - p - 1) + "px";
                            var l = f.position(), M = l.left;
                            M += s ? 2 : f.outerWidth() - K.track_size(), L.style.left = parseInt(M) + "px", C && (L.style.left = parseInt(M - 2) + "px", L.style.top = parseInt(l.top) + (g ? p - 2 : 0) + "px")
                        }
                    }
        ...
        var l = this, m = a(b), n = m.find(".nav-list").get(0);
        m.attr("data-sidebar-hover", "true"), g.push(m);
        var o = {}, p = ace.vars.old_ie, q = c.sub_hover_delay || ace.helper.intAttr(b, "data-sub-hover-delay") || 750, r = c.sub_scroll_style || m.attr("data-sub-scroll-style") || "no-track scroll-thin", s = !1;
        d && (q = parseInt(Math.max(q, 2500)));
        ...
        this.update_vars = function () {
            ...
            0 != l.dirty && (l.dirty = !1, m.find(".submenu").each(function () {
            ...
            }), m.find(".hover-show").removeClass("hover-show hover-shown hover-flip"))
        }, this.updateStyle = function (a) {
            r = a, m.find(".submenu.ace-scroll").ace_scroll("update", {styleClass: a})
        }, this.changeDir = function (a) {
            s = "right" === a
        };
        var w = -1;
        e || m.on("hide.ace.submenu.sidebar_hover", ".submenu", function (b) {
            if (!(1 > w)) {
                b.stopPropagation();
        ...
        }), e || m.on("shown.ace.submenu.sidebar_hover hidden.ace.submenu.sidebar_hover", ".submenu", function () {
            if (!(1 > w)) {
        ...
        m.on(y, ".nav-list li, .sidebar-shortcuts", function () {
            if (o = m.ace_sidebar("vars"), !o.collapsible) {
                var b = a(this), c = !1, e = b.hasClass("hover"), g = b.find("> .submenu").get(0);
                if (!(g || this.parentNode == n || e || (c = b.hasClass("sidebar-shortcuts"))))return void(g && a(g).removeClass("can-scroll"));
                var h = g, j = !1;
                if (h || this.parentNode != n || (h = b.find("> a > .menu-text").get(0)), !h && c && (h = b.find(".sidebar-shortcuts-large").get(0)), !(h && (j = f(h, "absolute")) || e))return void(g && a(g).removeClass("can-scroll"));
                var q = i(this);
                if (g)if (j) {
                    l.dirty = !0;
                    var r = ace.helper.scrollTop();
                    if (!q.is_visible() || !d && r != x || p)if (a(g).addClass("can-scroll"), p || d) {
                        var s = this;
                        setTimeout(function () {
                            k.call(s, g)
                        }, 0)
            ...
            o = m.ace_sidebar("vars"), o.collapsible || a(this).hasClass("hover-show") && i(this).hideDelay()
        })
    }
    ...
    var d = function (b) {
        ...
        this.reload = function () {
            ...
            var c = this.$box, d = c.find(".widget-body"), e = null, f = "undefined" != typeof a ? a : c.hasClass("collapsed") ? "show" : "hide", g = "show" == f ? "shown" : "hidden";
    ...
    var b = function () {
        function b() {
            function b(a, b) {
                c.find("." + a).removeClass(a).addClass("tmp-rtl-" + a).end().find("." + b).removeClass(b).addClass(a).end().find(".tmp-rtl-" + a).removeClass("tmp-rtl-" + a).addClass(b)
            }

            var c = a(document.body);
            c.toggleClass("rtl").find(".dropdown-menu:not(.datepicker-dropdown,.colorpicker)").toggleClass("dropdown-menu-right").end().find(".pull-right:not(.dropdown-menu,blockquote,.profile-skills .pull-right)").removeClass("pull-right").addClass("tmp-rtl-pull-right").end().find(".pull-left:not(.dropdown-submenu,.profile-skills .pull-left)").removeClass("pull-left").addClass("pull-right").end().find(".tmp-rtl-pull-right").removeClass("tmp-rtl-pull-right").addClass("pull-left").end().find(".chosen-select").toggleClass("chosen-rtl").next().toggleClass("chosen-rtl"), b("align-left", "align-right"), b("no-padding-left", "no-padding-right"), b("arrowed", "arrowed-right"), b("arrowed-in", "arrowed-in-right"), b("tabs-left", "tabs-right"), b("messagebar-item-left", "messagebar-item-right"), a(".modal.aside-vc").ace_aside("flip").ace_aside("insideContainer"), a(".fa").each(function () {
            ...
            var d = c.hasClass("rtl");
            d ? (a(".scroll-hz").addClass("make-ltr").find(".scroll-content").wrapInner('<div class="make-rtl" />'), a(".sidebar[data-sidebar-hover=true]").ace_sidebar_hover("changeDir", "right")) : (a(".scroll-hz").removeClass("make-ltr").find(".make-rtl").children().unwrap(), a(".sidebar[data-sidebar-hover=true]").ace_sidebar_hover("changeDir", "left")), a.fn.ace_scroll && a(".scroll-hz").ace_scroll("reset");
            try {
                var e = a("#piechart-placeholder");
                if (e.length > 0) {
                    var f = a(document.body).hasClass("rtl") ? "nw" : "ne";
                    e.data("draw").call(e.get(0), e, e.data("chart"), f)
                }
            } catch (g) {
            }
            ace.helper.redraw(document.body, !0)
        }
        ...
        if (0 == a("#ace-rtl-stylesheet").length) {
            ...
            a.ajax({url: e}).done(function () {
                ...
                d.length > 0 ? a.insertAfter(d) : c.length > 0 ? a.insertAfter(c.last()) : a.appendTo("head"), a.attr("href", e), b()
            })
        } else b()
    }
}(jQuery), function (a) {
    ...
    a("#skin-colorpicker").on("change", function () {
        ...
        if (0 == a("#ace-skins-stylesheet").length) {
            ...
            a.ajax({url: e}).done(function () {
                ...
                d.length > 0 ? a.insertAfter(d.last()) : a.appendTo("head"), a.attr("href", e), b(c)
            })
        } else b(c)
    })
}(jQuery), function (a) {
